...
add-highlighter shared/todotxt/ regex "^x ([^\n]+)" 0:comment             # done items
add-highlighter shared/todotxt/ regex "^\(A\) ([^\n]+)" 0:TodoPrioA   # priority (A)
add-highlighter shared/todotxt/ regex "^\(B\) ([^\n]+)" 0:TodoPrioB   # priority (B)
add-highlighter shared/todotxt/ regex "^\(C\) ([^\n]+)" 0:TodoPrioC   # priority (C)
add-highlighter shared/todotxt/ regex "([^:|^ ]+:)([^ |^\n]+)" 0:value 1:type # key:value
add-highlighter shared/todotxt/ regex "(\+[^\+|^ |^\n]+)" 0:keyword       # +project
add-highlighter shared/todotxt/ regex "(@[^\+|^ |^\n]+)" 0:meta           # @context
add-highlighter shared/todotxt/ regex "(\d{4}-\d{2}-\d{2})" 0:TodoDate       # date

hook -group todotxt-highlight global WinSetOption filetype=todotxt %{ add-highlighter window/todotxt ref todotxt }
